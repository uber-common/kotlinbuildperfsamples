...
class SampleUnionJsonAdapter(moshi: Moshi) : JsonAdapter<SampleUnion>() {
    private val options: JsonReader.Options = JsonReader.Options.of("type", "option1", "option2",
            "option3")

    private val sampleUnionUnionTypeAdapter: JsonAdapter<SampleUnionUnionType> =
            moshi.adapter<SampleUnionUnionType>(SampleUnionUnionType::class.java,
            kotlin.collections.emptySet(), "type")

    private val nullableStringAdapter: JsonAdapter<String?> =
            moshi.adapter<String?>(String::class.java, kotlin.collections.emptySet(), "option1")

    private val nullableBooleanAdapter: JsonAdapter<Boolean?> =
            moshi.adapter<Boolean?>(Boolean::class.javaObjectType, kotlin.collections.emptySet(),
            "option2")
    ...
    override fun fromJson(reader: JsonReader): SampleUnion {
        ...
        while (reader.hasNext()) {
            when (reader.selectName(options)) {
                0 -> type = sampleUnionUnionTypeAdapter.fromJson(reader) ?:
                        throw JsonDataException("""Non-null value 'type' was null at ${reader.path}""")
                1 ->  {
