...
class SampleExceptionJsonAdapter(moshi: Moshi) : JsonAdapter<SampleException>() {
    private val options: JsonReader.Options = JsonReader.Options.of("optionalField1",
            "optionalField2", "requiredEnumField", "optionalField3", "optionalField4",
            "optionalField5", "optionalField6")

    private val nullableLongAdapter: JsonAdapter<Long?> =
            moshi.adapter<Long?>(Long::class.javaObjectType, kotlin.collections.emptySet(),
            "optionalField1")

    private val nullableDoubleAdapter: JsonAdapter<Double?> =
            moshi.adapter<Double?>(Double::class.javaObjectType, kotlin.collections.emptySet(),
            "optionalField2")

    private val sampleEnumAdapter: JsonAdapter<SampleEnum> =
            moshi.adapter<SampleEnum>(SampleEnum::class.java, kotlin.collections.emptySet(),
            "requiredEnumField")

    private val nullableTypedefBAdapter: JsonAdapter<TypedefB?> =
            moshi.adapter<TypedefB?>(TypedefB::class.java, kotlin.collections.emptySet(),
            "optionalField3")

    private val nullableStringAdapter: JsonAdapter<String?> =
            moshi.adapter<String?>(String::class.java, kotlin.collections.emptySet(),
            "optionalField4")

    private val nullableIntAdapter: JsonAdapter<Int?> =
            moshi.adapter<Int?>(Int::class.javaObjectType, kotlin.collections.emptySet(),
            "optionalField5")

    private val nullableTypedefDAdapter: JsonAdapter<TypedefD?> =
            moshi.adapter<TypedefD?>(TypedefD::class.java, kotlin.collections.emptySet(),
            "optionalField6")
    ...
    override fun fromJson(reader: JsonReader): SampleException {
        ...
        while (reader.hasNext()) {
            when (reader.selectName(options)) {
                ...
                2 -> requiredEnumField = sampleEnumAdapter.fromJson(reader) ?:
                        throw JsonDataException("""Non-null value 'requiredEnumField' was null at ${reader.path}""")
                3 ->  {
        ...
        var result = SampleException(
                requiredEnumField = requiredEnumField ?:
                        throw JsonDataException("""Required property 'requiredEnumField' missing at ${reader.path}"""))
        result = result.copy(
